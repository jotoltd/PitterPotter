...
function getTimeSlots(date: Date, closures: ClosureDates): string[] {
  ...
  if (day >= 2 || day === 0 || (day === 1 && isHoliday)) {
    ...
    return filterPastSlots(getSlots('painting', 'Putney', dayType), date);
  }
  return [];
}
...
export default function PutneyView({ setCurrentPage, adminMode = false }: PutneyViewProps) {
  ...
  const handleBookDate = async () => {
    ...
    const remaining = await getRemainingCapacity('Putney', format(date, 'yyyy-MM-dd'), time, 'painting');
  ...
  const closedDatesAsDate = getClosedDatesForStudio(closures.closedDates, 'Putney').map(d => new Date(d + 'T00:00:00'));
